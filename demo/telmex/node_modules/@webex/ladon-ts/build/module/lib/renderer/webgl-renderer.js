...
import { bilinearUpsample as g } from "../util.js";
class l {
  constructor(t, i) {
...
`;
    ...
    if (e)
      ...
      s || r.errorAndThrow("Output canvas context must be webgl2 or 2d"), this.output_canvas_2d = s, this.intermediate_canvas = document.createElement("canvas"), this.intermediate_canvas.height = this.config.input.height, this.intermediate_canvas.width = this.config.input.width;
      const h = this.intermediate_canvas.getContext("webgl2", {
        antialias: !0
      });
      h || r.errorAndThrow("Error getting output context webgl"), this.gl = h;
    }
  ...
  initTexturesAndBuffers() {
    ...
    t || r.errorAndThrow("Error creating vertex array"), this.gl.bindVertexArray(t);
    const i = this.gl.createBuffer();
    i || r.errorAndThrow("Error creating buffer"), this.gl.bindBuffer(this.gl.ARRAY_BUFFER, i), this.gl.bufferData(this.gl.ARRAY_BUFFER, this.verts, this.gl.STATIC_DRAW), this.gl.vertexAttribPointer(0, 3, this.gl.FLOAT, !1, 0, 0), this.gl.enableVertexAttribArray(0), this.gl.viewport(0, 0, this.config.input.width, this.config.input.height);
  }
  ...
  setSharedUniforms(t) {
    ...
    i || r.errorAndThrow('Error finding "u_resolution" uniform'), this.gl.uniform2f(i, this.config.input.width, this.config.input.height);
    const e = this.gl.getUniformLocation(t, "u_image");
    e || r.errorAndThrow('Error finding "u_image" uniform'), this.gl.activeTexture(this.gl.TEXTURE0), this.gl.bindTexture(this.gl.TEXTURE_2D, this.image_texture), this.gl.uniform1i(e, 0);
