...
exports.SystemInfoEvents = void 0;
(function (SystemInfoEvents) {
    SystemInfoEvents["CpuPressureStateChange"] = "cpu-pressure-state-change";
})(exports.SystemInfoEvents || (exports.SystemInfoEvents = {}));
class PressureObserverHelper extends events.EventEmitter {
    constructor() {
        super();
    ...
    handleStateChange(records) {
        records.forEach((record) => {
            if (record.source === 'cpu' && record.state !== this.lastCpuPressure) {
                ...
                this.emit(exports.SystemInfoEvents.CpuPressureStateChange, record.state);
            }
        });
    }
    getCpuPressure() {
...
class SystemInfo {
    ...
    static getCpuPressure() {
        if (!SystemInfo.isPressureObserverSupported()) {
            return undefined;
        }
        return pressureObserverHelper.getCpuPressure();
    }
    static onCpuPressureChange(callback) {
        if (!SystemInfo.isPressureObserverSupported()) {
            return;
        }
        pressureObserverHelper.on(exports.SystemInfoEvents.CpuPressureStateChange, callback);
        const state = SystemInfo.getCpuPressure();
        if (state !== undefined) {
            callback(state);
        }
    }
    static offCpuPressureChange(callback) {
        if (!SystemInfo.isPressureObserverSupported()) {
            return;
        }
        pressureObserverHelper.off(exports.SystemInfoEvents.CpuPressureStateChange, callback);
    }
    static getNumLogicalCores() {
        return navigator.hardwareConcurrency;
    }
...
class WebCapabilities {
    ...
    static isCapableOfReceivingVideoCodec(mimeType) {
        ...
        return codecs.some((codec) => codec.mimeType === mimeType)
            ? exports.CapabilityState.CAPABLE
            : exports.CapabilityState.NOT_CAPABLE;
    }
