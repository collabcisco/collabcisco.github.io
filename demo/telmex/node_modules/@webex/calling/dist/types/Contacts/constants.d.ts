export declare const CONTACTS_CLIENT = "ContactsClient";
export declare const CONTACT_FILTER = "contacts";
export declare const GROUP_FILTER = "groups";
export declare const ENCRYPT_FILTER = "encrypt";
export declare const USERS = "Users";
export declare const DEFAULT_GROUP_NAME = "Other contacts";
export declare const CONTACTS_SCHEMA = "urn:cisco:codev:identity:contact:core:1.0";
export declare const SCIM_ID_FILTER = "id eq";
export declare const OR = " or ";
export declare enum encryptedFields {
    ADDRESS_INFO = "addressInfo",
    AVATAR_URL = "avatarURL",
    COMPANY = "companyName",
    DISPLAY_NAME = "displayName",
    EMAILS = "emails",
    FIRST_NAME = "firstName",
    LAST_NAME = "lastName",
    PHONE_NUMBERS = "phoneNumbers",
    SIP_ADDRESSES = "sipAddresses",
    TITLE = "title"
}
export declare const METHODS: {
    GET_CONTACTS: string;
    CREATE_NEW_ENCRYPTION_KEY_URL: string;
    FETCH_ENCRYPTION_KEY_URL: string;
    FETCH_DEFAULT_GROUP: string;
    CREATE_CONTACT_GROUP: string;
    DELETE_CONTACT_GROUP: string;
    CREATE_CONTACT: string;
    DELETE_CONTACT: string;
};
//# sourceMappingURL=constants.d.ts.map